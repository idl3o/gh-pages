...
foreach ($tool in $requiredTools.Keys) {
    if (-not (Get-Command $tool -ErrorAction SilentlyContinue)) {
        Write-Host "✗ $tool is not installed or not in PATH. $($requiredTools[$tool])" -ForegroundColor Red
            ...
            if (Get-Command "actionlint" -ErrorAction SilentlyContinue) {
                ...
                foreach ($file in $workflowFiles) {
                    try {
                        ...
                        Write-Host "✓ Basic YAML validation passed for $($file.Name)" -ForegroundColor Green
                    }
                    catch {
                        Write-Host "✗ YAML parsing failed for $($file.Name): $_" -ForegroundColor Red
                        $ErrorCount++
                    }
                }
            }
