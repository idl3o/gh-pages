...
class PaymentIntegration {
  ...
  async initialize(web3, options = {}) {
    ...
    try {
      ...
      const contractAddresses = {
        ...
        contentPayment:
          options.contentPaymentAddress || '0x2345678901234567890123456789012345678901',
      ...
      switch (currency.toUpperCase()) {
        case 'STREAM':
          ...
          if (parseFloat(balance) < parseFloat(amount)) {
            throw new Error(
              `Insufficient STREAM balance. Required: ${amount}, Available: ${balance}`
            );
          }
        ...
        case 'ETH':
          ...
          // Execute payment
          txReceipt = await this.contracts.contentPayment.methods
            .payWithEth(creatorAddress, contentId, web3.utils.asciiToHex(JSON.stringify(metadata)))
            .send({
  ...
  getPaymentHistory(options = {}) {
    const { type, limit = 10, offset = 0, sortBy = 'timestamp', sortDir = 'desc' } = options;
    ...
    try {
      ...
      if (subscription.active) {
        return {
          ...
          daysLeft: Math.ceil(
            (parseInt(subscription.expiryDate) * 1000 - Date.now()) / (24 * 60 * 60 * 1000)
          )
        };
      } else {
        return {
