...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function updateMiner() {
    ...
    const movementSpeed = miner.speed * miner.speedBoost;
  ...
  function isColliding(rect, obj, isCircle = false) {
    if (isCircle) {
      ...
      if (dx > rect.width / 2 + obj.radius) {
        return false;
      }
      if (dy > rect.height / 2 + obj.radius) {
        return false;
      }
  ...
  function drawEffects() {
    ...
    effects.forEach(effect => {
      if (effect.type === 'score' || effect.type === 'damage') {
        ...
        ctx.fillStyle =
          effect.type === 'score' ? `rgba(0, 255, 0, ${opacity})` : `rgba(255, 0, 0, ${opacity})`;
  ...
  function drawBlockProgress() {
    ...
    ctx.textAlign = 'center';
    ctx.fillText(
      `Mining block: ${Math.round(progress * 100)}%`,
      canvas.width / 2,
      y + barHeight + 15
    );
  }
  ...
  function updateLeaderboard(newScore) {
    ...
    if (
      newScore > 0 &&
      (leaderboard.length < 10 || newScore > leaderboard[leaderboard.length - 1].score)
    ) {
  ...
  canvas.addEventListener(
    ...
    e => {
      ...
      miner.x = Math.max(
        0,
        Math.min(
          canvas.width - miner.width,
          touchX - canvas.getBoundingClientRect().left - miner.width / 2
        )
      );
      miner.y = Math.max(
        0,
        Math.min(
          canvas.height - miner.height,
          touchY - canvas.getBoundingClientRect().top - miner.height / 2
        )
      );
    },
    { passive: false }
