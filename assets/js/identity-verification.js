...
class DecentralizedIdentity {
  ...
  async verifyIdentity() {
    ...
    const proofValid =
      this.verifiedCredentials.proof &&
      this.verifiedCredentials.proof.type === 'Web3Signature2023' &&
      this.verifiedCredentials.proof.verificationMethod.includes(
        this.verifiedCredentials.credentialSubject.id
      );
  ...
  async _sha256Simulate(input) {
    ...
    for (let i = 0; i < 64; i++) {
      const index =
        (inputSum + i + this.entropy.charCodeAt(i % this.entropy.length)) % characters.length;
