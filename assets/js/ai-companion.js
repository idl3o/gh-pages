...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const responses = {
    blockchain:
      "Blockchain is a distributed ledger technology that records transactions across many computers. It's the foundation of cryptocurrencies like Bitcoin and Ethereum, ensuring no single entity has control over the entire network. Each block contains a timestamp and link to the previous block, forming a chain.",
    wallets:
      'Crypto wallets are digital tools that allow you to store, send, and receive cryptocurrencies. They come in different forms: software wallets (mobile/desktop apps), hardware wallets (physical devices), and web wallets (browser-based). For maximum security, hardware wallets like Ledger or Trezor are recommended.',
    tokens:
      "Tokens are digital assets built on existing blockchains. Utility tokens provide access to a project's services, security tokens represent investment contracts, and governance tokens give voting rights in DAOs. Our platform uses the STREAM token for content monetization and governance.",
    streaming:
      'Web3 streaming uses blockchain to give creators direct control and revenue. Content is stored on decentralized networks like IPFS, payments happen through smart contracts, and viewers can support creators directly without platforms taking large cuts.',
  ...
  function generateResponse(userText) {
    ...
    if (
      ...
      text.includes('decentralized storage')
    ) {
    ...
    if (
      (text.includes('identity') || text.includes('verification') || text.includes('credential')) &&
  ...
  async function sendMessage() {
    ...
    const thinkingTime =
      baseThinkingTime + variableTime * messageComplexity * (0.7 + Math.random() * 0.6);
  ...
  function clearChat() {
    ...
    // Add welcome message
    addMessage(
      "Hello! I'm your Web3 assistant. I can help you understand blockchain technology, manage your crypto assets, and optimize your content streaming. What would you like to know about today?"
    );
  }
  ...
  async function connectWallet() {
    try {
      if (window.ethereum) {
        ...
        // Acknowledge in chat
        addMessage(
          `I've detected your connected wallet. I can now provide personalized assistance with your assets and content. Feel free to ask about your token balance or content performance.${identityMessage}`
        );
      } else {
        throw new Error(
      ...
      addMessage(
        `I couldn't connect to your wallet: ${error.message}. Please try again or make sure you have MetaMask installed.`
      );
    }
  }
    ...
    userInput.addEventListener('blur', async function () {
      ...
      try {
        ...
        if (result.moderated && result.metadata.severity === 'critical') {
          addSystemMessage(
            '⚠️ Warning: Your message appears to contain sensitive information such as private keys or addresses. Please be careful not to share such information in conversations.'
          );
        }
      } catch (error) {
