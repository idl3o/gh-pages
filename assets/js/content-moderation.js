/**
 * Content Safety System
 * Web3 Educational Content Platform
 *
 * Uses deterministic hashing with secure keys to protect sensitive information
 * and ensure educational content maintains appropriate standards
 */

class ContentSafetyService {
  constructor() {
    this.initialized = false;
    this.safetyKey = 'qah6uedmaxredactk8o';
    this.sensitivePatterns = null;
    this.safetyLevel = 'standard';
    this.safetyStats = {
      processed: 0,
      flagged: 0,
      redacted: 0
    };
  }

  /**
   * Initialize the content safety system
   * @param {Object} options - Configuration options
   * @returns {Promise<boolean>} - Whether initialization succeeded
   */
  async initialize(options = {}) {
    if (this.initialized) return true;

    try {
      console.log('Initializing content safety system...');

      // Set configuration options
      this.safetyLevel = options.level || 'standard';

      // Generate patterns based on the safety key
      await this.generatePatterns();

      this.initialized = true;
      console.log(`Content safety initialized at ${this.safetyLevel} level`);

      return true;
    } catch (error) {
      console.error('Failed to initialize safety system:', error);
      return false;
    }
  }

  /**
   * Generate detection patterns for sensitive information
   * @private
   */
  async generatePatterns() {
    // Create a simple hash from the key
    const keyHash = await this.simpleHash(this.safetyKey);

    // Define patterns for different types of sensitive information
    this.sensitivePatterns = {
      // PII detection (simplified)
      pii: [
        /\b\d{3}[-.]?\d{2}[-.]?\d{4}\b/g, // SSN patterns
        /\b\d{16}\b/g, // Credit card numbers (simplified)
        /\b[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}\b/gi // Email addresses
      ],

      // Blockchain security risks
      blockchain: [
        /\b0x[a-fA-F0-9]{40}\b/g, // Ethereum addresses
        /\b[5KL][1-9A-HJ-NP-Za-km-z]{50,51}\b/g, // Private key patterns (simplified)
        new RegExp(`${keyHash.substring(0, 8)}[a-zA-Z0-9]{8,32}`, 'g') // Custom pattern based on key
      ],

      // Sensitive terminology
      sensitive: [/\b(password|pwd|secret|key|mnemonic|seed phrase)\b/gi]
    };
  }

  /**
   * Process and ensure content safety
   * @param {string} content - Content to process
   * @param {Object} options - Safety options
   * @returns {Object} - Processed content and metadata
   */
  async processContent(content, options = {}) {
    if (!this.initialized) {
      await this.initialize();
    }

    this.safetyStats.processed++;

    // Default options
    const settings = {
      mode: options.mode || 'redact', // redact, flag, allow
      context: options.context || 'educational-content',
      preserveLength: options.preserveLength !== false
    };

    // Skip safety checks for trusted content
    if (options.trusted) {
      return {
        originalContent: content,
        modifiedContent: content,
        safetyApplied: false,
        metadata: { action: 'trusted' }
      };
    }

    let modifiedContent = content;
    let safetyApplied = false;
    const detections = [];

    // Apply patterns based on safety level
    const patternCategories = Object.keys(this.sensitivePatterns);

    for (const category of patternCategories) {
      // Skip certain categories based on safety level
      if (this.safetyLevel === 'low' && category !== 'blockchain') {
        continue;
      }

      const patterns = this.sensitivePatterns[category];

      for (const pattern of patterns) {
        // Reset lastIndex for regex reuse
        if (pattern instanceof RegExp) {
          pattern.lastIndex = 0;
        }

        const matches = modifiedContent.match(pattern);
        if (matches) {
          safetyApplied = true;

          detections.push({
            category,
            matches: matches.length,
            severity: this.getSeverity(category)
          });

          if (settings.mode === 'redact') {
            // Replace with censored content
            modifiedContent = modifiedContent.replace(pattern, match => {
              this.safetyStats.redacted++;

              // If this is educational content that needs to show the pattern
              // but not the actual sensitive data
              if (settings.context === 'educational-content' && category === 'blockchain') {
                return '[EXAMPLE VALUE]';
              }

              if (settings.preserveLength) {
                return '█'.repeat(match.length);
              } else {
                return '[REDACTED]';
              }
            });
          }
        }
      }
    }

    if (safetyApplied) {
      this.safetyStats.flagged++;
    }

    // Generate a verifiable hash of the result
    const resultHash = await this.simpleHash(modifiedContent + this.safetyKey);

    return {
      originalContent: content,
      modifiedContent: modifiedContent,
      safetyApplied: safetyApplied,
      metadata: {
        action: safetyApplied ? settings.mode : 'none',
        categories: detections.map(d => d.category),
        severity: this.getHighestSeverity(detections),
        verificationHash: resultHash.substring(0, 12)
      }
    };
  }

  /**
   * Get severity level for a specific category
   * @param {string} category - Category name
   * @returns {string} - Severity level
   * @private
   */
  getSeverity(category) {
    const severityMap = {
      pii: 'high',
      blockchain: 'critical',
      sensitive: 'medium'
    };

    return severityMap[category] || 'low';
  }

  /**
   * Get highest severity from a list of detections
   * @param {Array} detections - List of detections
   * @returns {string} - Highest severity
   * @private
   */
  getHighestSeverity(detections) {
    const severityLevels = {
      critical: 4,
      high: 3,
      medium: 2,
      low: 1
    };

    let highestLevel = 'none';
    let highestScore = 0;

    for (const detection of detections) {
      const score = severityLevels[detection.severity] || 0;
      if (score > highestScore) {
        highestScore = score;
        highestLevel = detection.severity;
      }
    }

    return highestLevel;
  }

  /**
   * Get safety statistics
   * @returns {Object} - Current safety stats
   */
  getStatistics() {
    return { ...this.safetyStats };
  }

  /**
   * Change the safety level
   * @param {string} level - New level (low, standard, high, strict)
   */
  setSafetyLevel(level) {
    const validLevels = ['low', 'standard', 'high', 'strict'];

    if (validLevels.includes(level)) {
      this.safetyLevel = level;
      console.log(`Content safety level set to: ${level}`);
      return true;
    }

    return false;
  }

  /**
   * Simple hashing function for demonstration purposes
   * @param {string} input - String to hash
   * @returns {Promise<string>} - Hashed string
   * @private
   */
  async simpleHash(input) {
    // In a real implementation, use a proper crypto hash function
    // This is a placeholder implementation

    let hash = 0;
    const str = input + this.safetyKey;

    for (let i = 0; i < str.length; i++) {
      const char = str.charCodeAt(i);
      hash = (hash << 5) - hash + char;
      hash = hash & hash; // Convert to 32bit integer
    }

    // Convert to hex string
    const hashHex = (hash >>> 0).toString(16);
    return hashHex.padStart(8, '0');
  }
}

// Create global singleton instance
window.contentSafety = new ContentSafetyService();
