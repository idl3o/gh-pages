...
void initialize(AppState* state) {
    ...
    for (int i = 0; i < MAX_PARTICLES; i++) {
        if (i < 30) {  // Only activate 30 particles initially
    ...
    for (int i = 0; i < 10; i++) {  // Create 10 random connections
...
int create_new_particle(AppState* state, int type, float x, float y) {
    ...
    Particle* p = &state->particles[new_index];
    p->active = true;
    p->x = x;
    p->y = y;
    p->type = type;
    ...
    p->vy = 0.0f;
    p->highlighted = false;
    p->energy = 1.0f; // Full energy to start
...
// Search for particles by type or partial name
void search_particles(AppState* state, const char* search_term) {
    ...
    // Try to match by node type first
    ...
    for (int i = 0; i < MAX_PARTICLES; i++) {
        ...
        else if (strcasestr(state->particles[i].data, search_term) != NULL) {
            match = true;
        }

        if (match && state->search_count < MAX_SEARCH_RESULTS) {
            state->search_results[state->search_count++] = i;
        }

        // Stop if we've found enough results
        if (state->search_count >= MAX_SEARCH_RESULTS) break;
    }

    // Highlight search results
    for (int i = 0; i < state->search_count; i++) {
        int idx = state->search_results[i];
        state->particles[idx].highlighted = true;
        state->particles[idx].energy = 1.0f; // Full energy
    }
}

// Calculate network statistics
void calculate_network_stats(AppState* state, float delta_time) {
    // Only update stats periodically for performance
    state->stats.update_timer += delta_time;
    if (state->stats.update_timer < STATISTICS_UPDATE_INTERVAL) return;
    ...
    // Each connection is counted twice (once per end)
...
void draw_creation_mode_ui(AppState* state) {
    ...
    // Draw title
    SDL_Color title_color = {255, 80, 80, 255};
    render_text(state->renderer, state->font, "NODE CREATION MODE",
              20, 15, title_color, 1.5f);

    // Draw instruction
    SDL_Color text_color = {200, 200, 200, 255};
    render_text(state->renderer, state->font, "Select node type and click to place:",
              20, 45, text_color, 1.0f);

    // Draw node type options
    int start_x = 300;
    int y = 45;
    int spacing = 50;

    const char* node_types[] = {"Core", "Val", "Stor", "Gate", "Oracle",
                              "Bridge", "Relay", "Arch", "ID", "Client"};

    for (int i = 0; i < 10; i++) {
        int x = start_x + i * spacing;

        // Draw selection highlight
        if (i == state->creation_type) {
        ...
        render_text_centered(state->renderer, state->font, node_types[i],
                          x, y + 20, label_color, 0.8f);
    }
    ...
    if (state->mode == MODE_CREATION && state->creation_cooldown <= 0) {
        ...
        switch (state->creation_type) {
            case 0: hex_color = (SDL_Color){255, 100, 100, 150}; break;
            case 1: hex_color = (SDL_Color){100, 255, 100, 150}; break;
            case 2: hex_color = (SDL_Color){100, 100, 255, 150}; break;
            case 3: hex_color = (SDL_Color){255, 255, 100, 150}; break;
            case 4: hex_color = (SDL_Color){255, 100, 255, 150}; break;
            case 5: hex_color = (SDL_Color){100, 255, 255, 150}; break;
            case 6: hex_color = (SDL_Color){255, 150, 50, 150}; break;
            case 7: hex_color = (SDL_Color){150, 100, 200, 150}; break;
            case 8: hex_color = (SDL_Color){100, 200, 150, 150}; break;
            case 9: hex_color = (SDL_Color){200, 200, 150, 150}; break;
        }

        draw_hexagon(state->renderer, state->mouse_x, state->mouse_y, HEX_RADIUS, hex_color);
    }
}
...
void draw_search_mode_ui(AppState* state) {
    ...
    // Draw title
    SDL_Color title_color = {80, 180, 255, 255};
    render_text(state->renderer, state->font, "SEARCH MODE",
              20, 15, title_color, 1.5f);

    // Draw search box
    SDL_Rect search_rect = {20, 45, 300, 25};
    SDL_SetRenderDrawColor(state->renderer, 50, 50, 60, 255);
    SDL_RenderFillRect(state->renderer, &search_rect);

    SDL_SetRenderDrawColor(state->renderer, 120, 120, 140, 255);
    SDL_RenderDrawRect(state->renderer, &search_rect);

    // Draw search term
    SDL_Color text_color = {220, 220, 220, 255};
    if (strlen(state->search_term) > 0) {
        render_text(state->renderer, state->font, state->search_term,
                  search_rect.x + 10, search_rect.y + 5, text_color, 1.0f);
    } else {
        render_text(state->renderer, state->font, "Type to search...",
                  search_rect.x + 10, search_rect.y + 5,
                  (SDL_Color){150, 150, 150, 255}, 1.0f);
    }

    // Draw search results
    if (state->search_count > 0) {
        char result_text[64];
        sprintf(result_text, "Found %d results:", state->search_count);
        render_text(state->renderer,state->font, result_text,
                 340, 50, text_color, 1.0f);

        // Draw result indicators
        for (int i = 0; i < state->search_count; i++) {
            int idx = state->search_results[i];
            if (idx >= 0 && idx < MAX_PARTICLES && state->particles[idx].active) {
                float pulse = 0.5f + 0.5f * sin(state->pulse_state * 3.0f + i * 0.5f);
                ...
                // Draw highlight circle around the particle
                int cx = (int)state->particles[idx].x;
                int cy = (int)state->particles[idx].y;
                int radius = HEX_RADIUS + 10;
                int segments = 16;
                float angle_step = 2.0f * M_PI / segments;
                ...
                for (int j = 0; j < segments; j++) {
                    ...
                    int x1 = cx + (int)(cos(angle1) * radius);
                    int y1 = cy + (int)(sin(angle1) * radius);
    ...
    } else if (strlen(state->search_term) > 0) {
        render_text(state->renderer, state->font, "No results found",
                  340, 50, (SDL_Color){200, 100, 100, 255}, 1.0f);
    }

    // Draw instructions
    render_text(state->renderer, state->font, "Press ESC to exit search mode",
              WINDOW_WIDTH - 250, 50, (SDL_Color){150, 150, 150, 255}, 0.8f);
}

// Draw statistics panel
void draw_statistics_panel(AppState* state) {
    // Draw panel
    SDL_Rect panel_rect = {WINDOW_WIDTH - 280, 100, 260, 300};
    SDL_SetRenderDrawColor(state->renderer, 30, 30, 40, 220);
    SDL_RenderFillRect(state->renderer, &panel_rect);
    ...
    render_text_with_shadow(state->renderer, state->font, "NETWORK STATISTICS",
                         panel_rect.x + 20, panel_rect.y + 15,
                         title_color, (SDL_Color){0, 0, 0, 128}, 1, 1, 1.2f);

    // Draw content
    SDL_Color text_color = {220, 220, 220, 255};
    int y = panel_rect.y + 50;

    // Node counts
    char total_text[64];
    sprintf(total_text, "Total Nodes: %d", state->active_particles);
    render_text(state->renderer, state->font, total_text,
              panel_rect.x + 20, y, text_color, 1.0f);
    y += 25;

    // Connection stats
    char connections_text[64];
    sprintf(connections_text, "Total Connections: %d", state->stats.total_connections);
    render_text(state->renderer, state->font, connections_text,
              panel_rect.x + 20, y, text_color, 1.0f);
    y += 25;

    char avg_connections_text[64];
    sprintf(avg_connections_text, "Avg. Connections: %.1f", state->stats.average_connections);
    render_text(state->renderer, state->font, avg_connections_text,
              panel_rect.x + 20, y, text_color, 1.0f);
    y += 25;

    char density_text[64];
    sprintf(density_text, "Network Density: %.2f", state->stats.network_density);
    render_text(state->renderer, state->font, density_text,
              panel_rect.x + 20, y, text_color, 1.0f);
    y += 40;

    // Node type distribution
    render_text(state->renderer, state->font, "Node Distribution:",
              panel_rect.x + 20, y, title_color, 1.0f);
    y += 25;

    // Calculate the most populous node type for scaling
    int max_nodes = 0;
    for (int i = 0; i < 10; i++) {
        if (state->stats.nodes_by_type[i] > max_nodes) {
            max_nodes = state->stats.nodes_by_type[i];
        }
    }
    ...
    int chart_width = 200;
    int chart_height = 8;
    int chart_x = panel_rect.x + 70;

    for (int i = 0; i < 10; i++) {
        // Only show types with nodes
        if (state->stats.nodes_by_type[i] > 0) {
            // Label
            render_text(state->renderer, state->font, node_types[i],
                      panel_rect.x + 20, y, text_color, 0.8f);

            // Bar background
            SDL_Rect bar_bg = {chart_x, y + 3, chart_width, chart_height};
            SDL_SetRenderDrawColor(state->renderer, 60, 60, 70, 255);
            SDL_RenderFillRect(state->renderer, &bar_bg);

            // Bar value
            float ratio = max_nodes > 0 ? (float)state->stats.nodes_by_type[i] / max_nodes : 0;
            SDL_Rect bar_val = {chart_x, y + 3, (int)(chart_width * ratio), chart_height};

            // Color based on node type
            SDL_Color bar_color;
            switch (i) {
                case 0: bar_color = (SDL_Color){255, 100, 100, 255}; break; // Red
                case 1: bar_color = (SDL_Color){100, 255, 100, 255}; break; // Green
                case 2: bar_color = (SDL_Color){100, 100, 255, 255}; break; // Blue
                case 3: bar_color = (SDL_Color){255, 255, 100, 255}; break; // Yellow
                case 4: bar_color = (SDL_Color){255, 100, 255, 255}; break; // Magenta
                case 5: bar_color = (SDL_Color){100, 255, 255, 255}; break; // Cyan
                case 6: bar_color = (SDL_Color){255, 150, 50, 255}; break;  // Orange
                case 7: bar_color = (SDL_Color){150, 100, 200, 255}; break; // Purple
                case 8: bar_color = (SDL_Color){100, 200, 150, 255}; break; // Teal
                case 9: bar_color = (SDL_Color){200, 200, 200, 255}; break; // Light gray
            }

            SDL_SetRenderDrawColor(state->renderer, bar_color.r, bar_color.g, bar_color.b, bar_color.a);
            SDL_RenderFillRect(state->renderer, &bar_val);

            // Value text
            char count_text[8];
            sprintf(count_text, "%d", state->stats.nodes_by_type[i]);
            render_text(state->renderer, state->font, count_text,
                      chart_x + chart_width + 10, y, text_color, 0.8f);
    ...
    if (state->stats.most_connected_node >= 0 &&
        state->particles[state->stats.most_connected_node].active) {

        y += 15;
        render_text(state->renderer, state->font, "Most Connected Node:",
                  panel_rect.x + 20, y, title_color, 0.9f);
        y += 20;

        Particle* p = &state->particles[state->stats.most_connected_node];
        render_text(state->renderer, state->font, p->data,
                  panel_rect.x + 20, y, text_color, 0.9f);
    }
...
int main(int argc, char* args[]) {
    ...
    while (state.running) {
        ...
        // Update creation cooldown timer
